...
export async function handleStoreRequest(options: {
  ...
  const assetByKind = new Map<string, any>()
  for (const asset of assetList) {
    if (asset?.kind) assetByKind.set(asset.kind, asset)
  }
  ...
  const printableAsset = sizedAsset || latestRepaired
  const readyStatuses = new Set(['ready_to_pay', 'paid', 'dispatching', 'printing', 'done'])
  const wantsReadyPublish = statusOverride === 'ready' || visibilityOverride !== 'hidden'
  const quote = orderRow.quote_json || {}
  const hasQuote = Number.isFinite(Number(quote?.price_cents ?? quote?.total_cents)) && Number.isFinite(Number(quote?.minutes)) && Number.isFinite(Number(quote?.grams))
  const threeMf = assetByKind.get('three_mf')
  const preview = assetByKind.get('slicer_preview_png')
  const sliceData = assetByKind.get('slicedata') || assetByKind.get('gcode')

  if (wantsReadyPublish) {
    if (!readyStatuses.has(String(orderRow.status || ''))) {
      return NextResponse.json({
        error: 'source_order_not_printable',
        message: 'Catalog publish requires an order that has completed slice and quote.',
      }, { status: 409 })
    }
    if (!printableAsset) {
      return NextResponse.json({
        error: 'printable_asset_missing',
        message: 'Catalog publish requires a repaired or finalized printable STL.',
      }, { status: 409 })
    }
    if (!threeMf) {
      return NextResponse.json({ error: 'three_mf_missing', message: 'Catalog publish requires a sliced 3MF artifact.' }, { status: 409 })
    }
    if (!preview) {
      return NextResponse.json({ error: 'preview_missing', message: 'Catalog publish requires a slicer preview artifact.' }, { status: 409 })
    }
    if (!sliceData) {
      return NextResponse.json({ error: 'slice_evidence_missing', message: 'Catalog publish requires slice output evidence.' }, { status: 409 })
    }
    if (!hasQuote) {
      return NextResponse.json({ error: 'quote_missing', message: 'Catalog publish requires deterministic slice quote data.' }, { status: 409 })
    }
  }
  ...
  const sizedMeta = printableAsset.meta_json || {}
  ...
  if (productInsert.error || !productInsert.data?.id) throw productInsert.error ?? new Error('product_insert_failed')
    ...
    const copied = await copyOrDownloadAsset(supabase, printableAsset.url as string, storagePath, printableAsset.sha256 as string | null)
    ...
    const productAssetInsert = await supabase
      ...
      .insert({
        ...
        version: versionNumber,
        kind: 'repaired_stl',
        source_asset_id: printableAsset.id,
        storage_path: copied.storagePath,
        sha256: copied.sha256 ?? (printableAsset.sha256 as string | null) ?? null,
