...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const terminalStates = new Set(['stabilizing','slicing','ready_to_pay','paid','dispatching','printing'])
    ...
    await supabase.from('order_events').insert({ order_id: orderId, phase: 'stabilizing', message: 'User requested fabrication' })
    try {
